...
def detect_threat(brd)
  current_player_markers = brd.each_with_object([]) do |(key, value), arr|
    arr << key if value == PLAYER_MARKER 
  end
  threats = WINNING_LINES.each_with_object([]) do |line, arr|
    arr << (line - current_player_markers)
  end
  threats.keep_if { |arr| arr.size == 1 }
  if threats.empty? == false
    value = threats.flatten
  else
    nil
  end
end
