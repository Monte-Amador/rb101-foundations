...
def validate_user_input(str, type = 'string', *data_range )
  case type.downcase
  when 'integer'
    Integer(str) rescue false 
    if data_range.empty? == false
      p data_range
      if (data_range.join).include?(str)
        puts "works"
      elsif data_range.join.to_i == str
        puts "works"
      else
        puts "did not work"
        return nil
  ...
  when 'string'
    if data_range.empty? == false
      temp_string = data_range.join(' ')
      if temp_string.split.include?(str.downcase.strip)
        str.strip.downcase if temp_string.split.include?(str.downcase.strip)
      else
...
def ask_who_goes_first(str)
  loop do
    ...
    prompt who
    answer = gets.chomp
    if validate_user_input(answer, 'string', 'p c r') != nil
      str << validate_user_input(answer, 'string', 'p c r')
      break
    else
